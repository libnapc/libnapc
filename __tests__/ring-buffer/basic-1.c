...
TEST_CASE("should work as expected") {
	...
	// pop 3 values
	TEST_ASSERT(libnapc_RingBuffer_getAvailableBytes(&rbuffer) == 3);

	TEST_ASSERT(libnapc_RingBuffer_removeByte(&rbuffer, &tmp));
	TEST_ASSERT(tmp == 1);
	...
	TEST_ASSERT(libnapc_RingBuffer_getAvailableBytes(&rbuffer) == 1);
	...
	TEST_ASSERT(libnapc_RingBuffer_insertByte(&rbuffer, 7));
	TEST_ASSERT(memcmp(buffer, "\x06\x07\x03\x04\x05\xff", 6) == 0);
	TEST_ASSERT(libnapc_RingBuffer_getAvailableBytes(&rbuffer) == 4);

	TEST_ASSERT(libnapc_RingBuffer_insertByte(&rbuffer, 8));
	TEST_ASSERT(memcmp(buffer, "\x06\x07\x08\x04\x05\xff", 6) == 0);
	TEST_ASSERT(libnapc_RingBuffer_getAvailableBytes(&rbuffer) == 5);

	// should not work (buffer full)
	TEST_ASSERT(!libnapc_RingBuffer_insertByte(&rbuffer, 9));
	TEST_ASSERT(memcmp(buffer, "\x06\x07\x08\x04\x05\xff", 6) == 0);
	TEST_ASSERT(libnapc_RingBuffer_getAvailableBytes(&rbuffer) == 5);
}
