...
TEST_CASE("should work correctly") {
	...
	TEST_ASSERT(napc_Pool_claimElement(&pool) == 0);
	TEST_ASSERT(napc_Pool_claimElement(&pool) == 1);
	TEST_ASSERT(napc_Pool_claimElement(&pool) == 2);
	TEST_ASSERT(napc_Pool_claimElement(&pool) == 3);
	TEST_ASSERT(napc_Pool_claimElement(&pool) == 4);

	TEST_ASSERT(napc_Pool_getAddress(&pool, 0) == addr_element0);
	TEST_ASSERT(napc_Pool_getAddress(&pool, 1) == addr_element1);
	TEST_ASSERT(napc_Pool_getAddress(&pool, 2) == addr_element2);
	TEST_ASSERT(napc_Pool_getAddress(&pool, 3) == addr_element3);
	TEST_ASSERT(napc_Pool_getAddress(&pool, 4) == addr_element4);
